...
public:
	...
	std::vector<Node<T>*> shortestPath(const T& start, const T& end) {
		...
		/*
		typedef std::unordered_map<T, Node<T>*>::iterator it_type;
		for (it_type iterator = nodes.begin(); iterator != nodes.end(); ++iterator) {

		}
		*/

		Node<T>* currentNode = nodes[start];
		Node<T>* endNode = nodes[end];
		...
		std::deque< std::vector<Node<T>*> > allPartialPaths;
		...
		allPartialPaths.push_back(partialPath);
		...
		while (!allPartialPaths.empty()) {
			partialPath = allPartialPaths.front();
			currentNode = partialPath.back();
			allPartialPaths.pop_front();

			for (auto iter = currentNode->adjacents.begin(); iter != currentNode->adjacents.end(); iter++) {
				if ((*iter)->marked == false) {
					(*iter)->marked = true;

					std::vector<Node<T>*> currentPath(partialPath);
					...
					allPartialPaths.push_back(currentPath);
				}
			}
